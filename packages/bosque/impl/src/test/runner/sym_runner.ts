...
function runSymTest(
  ...
  test: SymTest,
  cpayload: object,
  cbpre: (test: SymTest | SymTestInternalChkShouldFail) => void,
  cb: (
    result: "pass" | "passlimit" | "fail" | "error",
    test: SymTest | SymTestInternalChkShouldFail,
    start: Date,
    end: Date,
    smttime: number,
    info?: string
...
function runSymTestShouldFail(
  exepath: string,
  verbose: boolean,
  test: SymTestInternalChkShouldFail,
  cpayload: object,
  cbpre: (test: SymTest | SymTestInternalChkShouldFail) => void,
  cb: (
...
function enqueueSymTestInternalChkShouldFail(
  exepath: string,
  verbose: boolean,
  test: SymTestInternalChkShouldFail,
  icppasm: any,
...
function generateTestResultCallback(
  exepath: string,
  verbose: boolean,
  winfo: {
    worklist: { test: SymTest | SymTestInternalChkShouldFail; cpayload: any }[];
    cpos: number;
    done: number;
  },
  cbpre: (test: SymTest | SymTestInternalChkShouldFail) => void,
    ...
    if (winfo.cpos < winfo.worklist.length) {
      ...
      if (ttest instanceof SymTest) {
        enqueueSymTest(
          ...
          verbose,
          ttest,
          winfo.worklist[winfo.cpos].cpayload,
          cbpre,
          generateTestResultCallback(exepath, verbose, winfo, cbpre, cb, cbdone)
        );
      } else {
        enqueueSymTestInternalChkShouldFail(
          exepath,
          verbose,
          ttest,
          winfo.worklist[winfo.cpos].cpayload,
...
function enqueueSymTests(
  exepath: string,
  tests: { test: SymTest | SymTestInternalChkShouldFail; cpayload: any }[],
  verbose: boolean,
  cbpre: (test: SymTest | SymTestInternalChkShouldFail) => void,
  cb: (
    result: "pass" | "passlimit" | "fail" | "error",
  ...
  for (let i = 0; i < Math.min(tests.length, PARALLEL_COUNT_SMT); ++i) {
    ...
    if (ttest instanceof SymTest) {
      enqueueSymTest(
        ...
        tests[i].cpayload,
        cbpre,
        generateTestResultCallback(exepath, verbose, shared_work_info, cbpre, cb, cbdone)
      );
    } else {
      enqueueSymTestInternalChkShouldFail(
        exepath,
