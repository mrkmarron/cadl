...
function runICPPTest(
  exepath: string,
  verbose: boolean,
  test: ICPPTest,
  icppjson: { code: object; args: any[]; main: string },
  cbpre: (test: ICPPTest) => void,
  cb: (
  ...
  try {
    ...
    const proc = exec(
      cmd,
      { env: { ...process.env, ICPP_OUTPUT_MODE: "json" } },
      (err, stdout, stderr) => {
        const end = new Date();
...
function generateTestResultCallback(
  exepath: string,
  verbose: boolean,
  winfo: { worklist: { test: ICPPTest; apiinfo: any; icppasm: any }[]; cpos: number; done: number },
  cbpre: (test: ICPPTest) => void,
  cb: (
    result: "pass" | "fail" | "error",
    test: ICPPTest,
  ...
  return (
    ...
    if (winfo.cpos < winfo.worklist.length) {
      enqueueICPPTest(
        exepath,
        verbose,
        winfo.worklist[winfo.cpos].test,
        winfo.worklist[winfo.cpos].apiinfo,
        winfo.worklist[winfo.cpos].icppasm,
        cbpre,
...
function enqueueICPPTests(
  exepath: string,
  tests: { test: ICPPTest; apiinfo: any; icppasm: any }[],
  verbose: boolean,
  cbpre: (test: ICPPTest) => void,
  cb: (
    result: "pass" | "fail" | "error",
