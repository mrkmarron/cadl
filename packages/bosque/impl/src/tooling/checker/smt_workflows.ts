...
const exepath = Path.join(
  bosque_dir,
  "/build/output/chk" + (process.platform === "win32" ? ".exe" : "")
);
...
function generateMASM(
  ...
  istestbuild: boolean,
  smallmodelonly: boolean,
  entrypointkey: MIRInvokeKey,
  entrypoint: { filename: string; names: string[] }
): { masm: MIRAssembly | undefined; errors: string[] } {
  ...
  return MIREmitter.generateMASM(
    BuildApplicationMode.ModelChecker,
    [coreconfig, usercode],
    buildlevel,
    istestbuild,
    [entrypointkey],
...
function generateSMTPayload(
  masm: MIRAssembly,
  istestbuild: boolean,
  vopts: VerifierOptions,
  errorTrgtPos: { file: string; line: number; pos: number },
  entrypoint: MIRInvokeKey
): string | undefined {
  try {
    return SMTEmitter.generateSMTPayload(
      masm,
      istestbuild,
      smtruntime,
      vopts,
...
function generateCheckerPayload(
  masm: MIRAssembly,
  istestbuild: boolean,
  smtasm: string,
  timeout: number,
  ...
  return {
    smt2decl: smtasm,
    timeout: timeout,
    apimodule: masm.emitAPIInfo([entrypoint], istestbuild),
    mainfunc: entrypoint,
  };
}

function runVEvaluatorAsync(
  cpayload: object,
  mode: SymbolicActionMode,
...
): { file: string; line: number; pos: number; msg: string }[] | undefined {
  try {
    const { masm, errors } = generateMASM(
      usercode,
      buildlevel,
      istestbuild,
      smallmodelonly,
      entrypoint.fkey,
      { filename: entrypoint.filename, names: [entrypoint.name] }
    );
    if (masm === undefined) {
    ...
    if (masm === undefined) {
      ...
      if (smtcode !== undefined) {
        if (smtonly) {
          ...
          const payload = generateCheckerPayload(
            masm,
            istestbuild,
            smtcode,
            timeout,
...
function workflowErrorCheckSingle(
  usercode: PackageConfig,
  buildlevel: BuildLevel,
  smallmodelonly: boolean,
  istestbuild: boolean,
...
function workflowPassCheck(
  ...
  try {
    ...
    if (masm === undefined) {
      ...
      const smtcode = generateSMTPayload(
        masm,
        istestbuild,
        vopts,
        { file: "[NO TARGET]", line: -1, pos: -1 },
        entrypoint.fkey
      );
      if (smtcode === undefined) {
        cb(JSON.stringify({ result: "error", info: "payload generation error" }));
...
function workflowEvaluate(
  ...
  try {
    const { masm } = generateMASM(usercode, buildlevel, istestbuild, false, entrypoint.fkey, {
